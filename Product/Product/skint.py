...
def reverse(dataset):
    ...
    # 先采用一个进行测试
    maxvalue = maxmodel.predict(allele).resize(56, 801, 1381)
    minvalue = minmodel.predict(allele).resize(56, 801, 1381)
    # 路径
    ...
    for i in range(56):
        ...
        prediction = np.array(maxmodel.predict(newdataset))  # 每轮结果

        rediction = np.array(minmodel.predict(newdataset))  # 每轮结果
        snowdepth = prediction  # 结果作为下一次预测的输入
        prediction.resize(len(glovar.lat), len(glovar.lon))
