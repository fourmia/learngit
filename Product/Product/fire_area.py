...
def clcfire(df, list2, list3):
    ...
    length = math.hypot(testarray[0][0], testarray[0][1])
    res = length *
    # 3个节点
...
def Node(QHpoints, distance=0.5):
    ...
    # 保存为List
    res,i = [],0
    for QHpoint in QHpoints:
        print(len(QHpoint))
        lon, lat, scan, track = QHpoint
        tmp = [QHpoint]
        for lonn, latt, scan, track in QHpoints[i+1:]:
            if (lonn > (lon-distance)) & (lonn < (lon+distance)) & (lat > (lat-distance)) & (lat < (lat+distance)):
                tmp.append([lonn, latt, scan, track])
                QHpoints.remove([lonn, latt, scan, track])
        i+=1
        res.append(tmp)
    return res
...
def clcfire(df, list2, list3):
    ...
    return res, res2, hull.area
'''
def clcfire(difflist, typecode=None):
    # 目前此函数仅仅只能针对一个list2和list3计算
    diffarray = np.array(difflist)
    if typecode == None:
        print("TypeCode Error!, Please input 1-3")
        raise TypeError
    elif typecode ==1:
        tmp = diffarray[...,-2:] * 1000
        res = tmp[...,0] * tmp[...,1]
    elif typecode ==2:
        tmp = np.diff(diffarray[...,1:2],axis=1)*100000  # 求得两个相邻点间的距离
        res = (tmp + diffarray[...,-2:][0][0][0]*1000) * (diffarray[...,-2:][0][0][1]*1000)
    else:
        tmp1 = ConvexHull(diffarray[...,:2][0] * 100000).area
        tmp2 = np.sum(diffarray[...,-2:][...,0] * diffarray[...,-2:][...,1] * pow(10, 6))
        res = np.max([tmp1, tmp2])
    return res
    '''
    res1 = diffarray[...,-2:] *1000
    np.piecewise(typecode,[typecode==1, typecode==2, typecode==3], [])
    '''
...
def main():
    ...
    area1 = clcfire(list1, typecode=1)
    area2 = clcfire(list2, typecode=2)
    area3 = clcfire(list3, typecode=3)
