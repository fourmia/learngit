...
class LeNet(nn.Module):
    ...
    def forward(self, x):
        ...
        #这里定义的forward不是调用，我们可以理解成数据流的方向，给net输入数据input会按照forward提示的流程进行处理和操作并输出数据
        x = x.float()
...
if __name__ == '__main__':
    ...
    # 加载标签
    with open(r'C:/Users/GJW/Desktop/outputsrain.pickle', 'rb') as f:
        datalabel = pickle.load(f)
    nt = torch.from_numpy(np.array(datalabel))
    print(nt.dtype)
    # 加载输入
    with open(r'C:/Users/GJW/Desktop/inputsrain.pickle', 'rb') as f:
        datainput = pickle.load(f)
    # datainput = np.load(r'C:/Users/GJW/Desktop/inputsrain.pickle', 'rb')
    uvg = torch.from_numpy(np.array(datainput))
    print(uvg.dtype)
